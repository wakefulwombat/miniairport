...
//複数のキー入力の流れに対して判定を行う
class CommandFlow : public RequiredFunc {
private:
...
public:
	...
	void initialize() override { this->checking_index = 0; this->time = 0; };
...
class KeyManager : public RequiredFunc {
private:
...
public:
	...
	virtual ~KeyManager() {}
	void initialize() override { this->accept_update_input = true; this->return_all_false = false; this->downed_time = 10000; this->time_counter = 0; this->check_keys.clear(); this->downed_keys.clear(); this->downed_keys_pre.clear(); }
	void update() override;
...
class KeyBoard : public KeyBoardOperationInterface<T_UserKeyID>, public KeyBoardEventInterface<T_UserKeyID>, public std::enable_shared_from_this<KeyBoard<T_UserKeyID, T_UserCommandID>>, public RequiredFunc {
private:
...
public:
	...
	void initialize() override { this->acceptUpdateInput(true); this->setReturnAllKeyFalse(false); this->key_mgr->initialize(); this->command_mgr->initialize(); }
	void update() override { this->key_mgr->update(); this->command_mgr->update(); }
...
class KeyBoardLogs : public KeyBoardLoggingInterface<T_LogID, T_UserKeyID>, public RequiredFunc {
private:
...
public:
	...
	void initialize() override { this->logs.clear(); }
	void update() override { for (auto it = this->logs.begin(); it != this->logs.end(); ++it) { if (!it->second->isLoggingNow()) continue; it->second->pushLog(); } }
...
template<typename T_UserKeyID, typename T_UserCommandID, typename T_LogID>
class KeyBoardManager : public RequiredFunc {
private:
...
public:
	...
	void initialize() override { for (int i = 0; i < 256; ++i) this->key_now[i] = 0; this->now->initialize(); this->logs->initialize(); }
	void update() override { GetHitKeyStateAll(this->key_now); this->now->update(); this->logs->update(); }
...
template<typename T_UserKeyID>
class MousePointerManager : public RequiredFunc {
private:
...
public:
	...
	void initialize() override { this->accept_update_input = true; this->return_all_false = false; this->showing_pointer = false; this->pointer_position_now = Vec2D(); this->pointer_position_pre = Vec2D(); this->wheel_rotation = 0; }
	void update() override { if (!this->accept_update_input) return; this->pointer_position_pre = this->pointer_position_now; this->pointer_position_now = this->get_pointer_position(); this->wheel_rotation += get_wheel_rotation(); }
	...
	bool isInSquare(Vec2D leftup_window, Vec2D rightdown_window) { if (this->return_all_false) return false; return this->pointer_position_now.isInSquare(leftup_window, rightdown_window); }
	bool isInCircle(Vec2D center_window, double r) { if (this->return_all_false) return false; return this->pointer_position_now.getDistance(center_window) < r; }
	bool isCrossIntoSquare(Vec2D leftup_window, Vec2D rightdown_window) { if (this->return_all_false) return false; return ((this->pointer_position_now.isInSquare(leftup_window, rightdown_window)) && (!this->pointer_position_pre.isInSquare(leftup_window, rightdown_window))); }
	bool isCrossIntoCircle(Vec2D center_window, double r) { if (this->return_all_false) return false; return ((this->pointer_position_now.getDistance(center_window) <= r) && (this->pointer_position_pre.getDistance(center_window) > r)); }
	bool isCrossOuttoSquare(Vec2D leftup_window, Vec2D rightdown_window) { if (this->return_all_false) return false; return (!(this->pointer_position_now.isInSquare(leftup_window, rightdown_window)) && (this->pointer_position_pre.isInSquare(leftup_window, rightdown_window))); }
	bool isCrossOutoCircle(Vec2D center_window, double r) { if (this->return_all_false) return false; return ((this->pointer_position_now.getDistance(center_window) > r) && (this->pointer_position_pre.getDistance(center_window) <= r)); }
};

template<typename T_UserKeyID, typename T_UserCommandID>
class Mouse : public MouseOperationInterface<T_UserKeyID>, public MouseEventInterface<T_UserKeyID>, public std::enable_shared_from_this<Mouse<T_UserKeyID, T_UserCommandID>>, public RequiredFunc {
private:
	std::shared_ptr<KeyManager<T_UserKeyID>> key_mgr;
	std::shared_ptr<CommandManager<T_UserCommandID>> command_mgr;
...
public:
	...
	void initialize() override { this->acceptUpdateInput(true); this->setReturnAllKeyFalse(false); this->key_mgr->initialize(); this->command_mgr->initialize(); this->pointer_mgr->initialize(); }
	void update() override { this->key_mgr->update(); this->pointer_mgr->update(); this->command_mgr->update(); }
...
template<typename T_UserKeyID, typename T_UserCommandID, typename T_LogID>
class MouseLogs : public MouseLoggingInterface<T_LogID, T_UserKeyID>, public RequiredFunc {
private:
...
template<typename T_UserKeyID, typename T_UserCommandID, typename T_LogID>
class MouseManager : public RequiredFunc {
private:
...
public:
	...
	void initialize() override { this->mouse_input = 0; this->mouse_pos = Vec2D(); this->wheel_rotation = 0; this->now->initialize(); this->logs->initialize(); }
	void update() override { this->mouse_input = GetMouseInput(); this->wheel_rotation = GetMouseWheelRotVol(); int x, y; GetMousePoint(&x, &y); this->mouse_pos = Vec2D(x, y); this->now->update(); this->logs->update(); }
...
template <typename T_UserKeyID>
class AnalogStickManager : public RequiredFunc {
private:
...
public:
	...
	void initialize() override { for (unsigned int i = 0; i < this->log_bef_count; ++i) { this->analog_stick_slope_raw[0].push_back(Vec2D()); this->analog_stick_slope_raw[1].push_back(Vec2D()); } }
	void update() override {
	...
	Direction4 getDirection4(AnalogStickLeftRight lr, double per_min = 0.01) { if (this->return_all_false) return Direction4::NEUTRAL; return this->analog_stick_slope_raw[(int)lr][0].toDirection4(per_min); }
	Direction8 getDirection8(AnalogStickLeftRight lr, double per_min = 0.01) { if (this->return_all_false) return Direction8::NEUTRAL; return this->analog_stick_slope_raw[(int)lr][0].toDirection8(per_min); }
	...
	bool isInDirection(AnalogStickLeftRight lr, Direction4 dir, double per_min = 0.01, double per_max = 1.0) { if (this->return_all_false) return false; if ((this->analog_stick_slope_raw[(int)lr][0].toNorm() < per_min) || (this->analog_stick_slope_raw[(int)lr][0].toNorm() > per_max)) return false; return this->analog_stick_slope_raw[(int)lr][0].toDirection4(per_min) == dir; }
	bool isInDirection(AnalogStickLeftRight lr, Direction8 dir, double per_min = 0.01, double per_max = 1.0) { if (this->return_all_false) return false; if ((this->analog_stick_slope_raw[(int)lr][0].toNorm() < per_min) || (this->analog_stick_slope_raw[(int)lr][0].toNorm() > per_max)) return false; return this->analog_stick_slope_raw[(int)lr][0].toDirection8(per_min) == dir; }
...
template<typename T_UserKeyID, typename T_UserCommandID>
class GamePad : public GamePadOperationInterface<T_UserKeyID>, public GamePadEventInterface<T_UserKeyID>, public std::enable_shared_from_this<GamePad<T_UserKeyID, T_UserCommandID>>, public RequiredFunc {
private:
	std::shared_ptr<KeyManager<T_UserKeyID>> key_mgr;
	std::shared_ptr<CommandManager<T_UserCommandID>> command_mgr;
...
public:
	...
	void initialize() override { this->acceptUpdateInput(true); this->setReturnAllKeyFalse(false); this->key_mgr->initialize(); this->command_mgr->initialize(); this->analog_mgr->initialize(); }
	void update() override { this->key_mgr->update(); this->command_mgr->update(); this->analog_mgr->update(); }
...
template<typename T_UserKeyID, typename T_UserCommandID, typename T_LogID>
class GamePadLogs : public GamePadLoggingInterface<T_LogID, T_UserKeyID>, public RequiredFunc {
private:
...
template<typename T_UserKeyID, typename T_UserCommandID, typename T_LogID>
class GamePadManager : public RequiredFunc {
private:
...
public:
	...
	void initialize() override { this->gamepad_input = 0; this->stick_slope[0] = Vec2D(); this->stick_slope[1] = Vec2D(); this->now->initialize(); this->logs->initialize(); }
	void update() override { this->gamepad_input = GetJoypadInputState(DX_INPUT_PAD1); DINPUT_JOYSTATE input; GetJoypadDirectInputState(DX_INPUT_PAD1, &input); this->stick_slope[0] = Vec2D(0.001*input.X, 0.001*input.Y); this->stick_slope[1] = Vec2D(0.001*input.Z, 0.001*input.Rz);  this->now->update(); this->logs->update(); }
